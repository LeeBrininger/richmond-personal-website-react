...
function DisplayImage(name: any) {
  switch (name.name) {
    case "CompTIA Security+":
      return (
        <img src={security} className="certification" alt="AWS Certification" />
      );
    case "AWS Certified Cloud Practitioner":
      return (
        <img src={aws} className="certification" alt="AWS Certification" />
      );
    default:
...
export default function CertificationTabPanel(props: TabPanelProps) {
  ...
  return (
    <div
      ...
      {...other}
    >
      {value === index && <DisplayImage name={children.name} />}
    </div>
  );
}
